...
config.load_env()
config.load_argv() # sys.argv will overwrite os.environ  # -ENV name=value -CFG name=value
...
if cloud_factory_ == 'AWS':
  try:
    s = urlopen('http://169.254.169.254/latest/meta-data/instance-id').read()
    if s:
    ...
    if s:
      ...
      s = urlopen('http://169.254.169.254/latest/meta-data/placement/availability-zone').read()
      if s:
        region_name = s.decode('latin-1')
        if region_name[-1] in 'abcdefghijk':
          region_name = region_name[:-1]   # cn-north-1a  --> cn-north-1
...
def application(environ, start_response):
  ...
  if handler:
    try:
      ...
      if ret:
        return ret
      else: status = 404  # not processed, means resource inexistent
    except Exception as e:
      traceback.print_exc(file=sys.stdout)  # for debug
      # Error.error('%s: %s' % (e.__class__.__name__,str(e)))
      if __LocalDbg__:
        lines = ''.join(traceback.format_exception(*sys.exc_info()))
        start_response(Error.status_code_desc(500),[('Content-Type', 'text/html; charset=utf-8')])
        return [ ('<pre>%s</pre>' % cgi.escape(lines,quote=True)).encode('utf-8') ]
      else: status = 500
  else: status = 404
...
class WSGIRouter:
  def __init__(self,handler_tuples):
    ...
    for regexp, handler in handler_tuples:
      ...
      handler_patterns = pattern_map.setdefault(handler, []) # if already exist, reuse old one
...
def binary_result(code, binary, response, headers=None):  # binary should be bytes
...
def http_input(environ, is_qs=True):
  try:
    if is_qs:
      ret = {}
      query_str = environ.get('QUERY_STRING','')  # request_arg is <class str>
      for (name,value) in urlparse.parse_qsl(query_str):  # value of parameter has unquoted
        tmp = ret.get(name,None)
        if tmp is None:
          ret[name] = value
        elif type(tmp) == list:
          tmp.append(value)
        else: ret[name] = [tmp,value]
      return ret
    else:
...
def make_query_items_(bItem, sFmt):
  ...
  ret = []
  bFmt = sFmt.split(','); fmtLen = len(bFmt)
  for item in bItem:
    b = []
    for i in range(fmtLen):
      fmt = bFmt[i]; v = item[i] if fmtLen > 1 else item
      if fmt == 'n':
        v = make_long(v)
      elif fmt == 'f':
        v = float(v)
...
def adjust_msg_format(param, fmt):
  try:
    for (attr,data_type) in fmt:    # data_type can be any of 'nfsbx' or compose with '[]'
      ...
      else: return False      # unknown data type
...
def server_sign(sequence, pks_out, last_uocks, version, tx_in, tx_out, lock_time):
...
def root_(environ, method, response):
  if method == 'GET':
    ...
    sHtml = b'OK'
    response('200 OK',[('Content-Type','text/html'),('Content-Length',str(len(sHtml)))])
    return [sHtml]
  ...
  if action == 'account':
          ...
          if DbLocker.wait_enter(15):
            ...
            try:
              if node.in_catching():
                ...
                addr2 = util.base58.decode_check(account)  # the input of decode_check can be str or bytes
                coin_hash = addr2[3:]  # coin_hash: pub_hash32 + cointype
                utxos = node.list_unspent(coin_hash,1024,0,0,uock,uock2)  # limit = 1024
                
                bOut  = [protocol.format.UockValue(u.uock,u.value,u.height,u.vcn) for u in utxos]
                bin_msg = protocol.AccState(node._link_no,int(time.time()),account,1024,bOut)
                ret = binary_result(200,bin_msg.binary(node.coin.magic),response)
            except Exception as e:
              sErr = str(e)
              traceback.print_exc()
    ...
    if method == 'GET':
      ...
      try:
        ...
        if not bk_hash:
          bk_hash = _0
        else: bk_hash = unhexlify(bk_hash)
        ...
        if DbLocker.wait_enter(15):
          ...
          try:
            if node.in_catching():
              ...
              blocks = []; exists = []; highest = node.chain_height
              if bk_hash != _0:
              ...
              bin_msg = protocol.ReplyHeaders(node._link_no,hh,tt,bb)
              ret = binary_result(200,bin_msg.binary(node.coin.magic),response)
          except Exception as e:
            sErr = str(e)
            traceback.print_exc()
          ...
          if DbLocker.wait_enter(15):
            ...
            try:
              if node.in_catching():
                ...
                if uocks:
                  utxos = node.list_unspent2(uocks)  # owner txn of every uock has been cached
                else:
                  addr2 = util.base58.decode_check(account)  # the input of decode_check can be str or bytes
                  coin_hash = addr2[3:]  # coin_hash: pub_hash32 + cointype
                  utxos = node.list_unspent(coin_hash,num,0,0,-1) # limit = num, uock=-1 means get from lastest one, owner txn cached
                
                hi = []; idx = []; txns = []
                for u in utxos:
                  hi.append(u.height)
                  idx.append((((u.uock >> 20) & 0xffff) << 16) | (u.uock & 0xffff))
                  txns.append(u.get_txn())
                
                bin_msg = protocol.UtxoState(node._link_no,hi,idx,txns)
                ret = binary_result(200,bin_msg.binary(node.coin.magic),response)
            except Exception as e:
              sErr = str(e)
              traceback.print_exc()
...
def testing2_(environ, method, response, action):
  ...
  if action == 'debug':  # for testing
    if method == 'POST':
      param = http_input(environ)
      if _dbg_session != param.get('sid','') or (time.time() - _dbg_start_at) > 3600: # 3600 is 1 hour
        response('401 Unauthorized',[])
        return [ b'' ]
      
      data = environ['wsgi.input'].read(int(environ.get('CONTENT_LENGTH',0))) # data is bytes
    ...
    if method == 'POST':
      ...
      if data and sh == data.get('session'):
        ...
        wa = wallet.Address(pub_key=DBG_PUBKEY,vcn=0,coin_type=b'\x00')
...
def check_run_patch():
  sFile = os.path.join(local_datadir_,'patch.py')  # not depends __LocalDbg__
...
def exit_node():    # exit process but no call ec2_reboot()  # exit all threads except MainThread
  if __LocalDbg__:
    ...
    for i in range(300):  # max wait 300 seconds
      if (not db_backup or not db_backup._active) and (not mining._poet_thread or not mining._poet_thread._active):
        succ = True
        break
      time.sleep(1)
...
if app_type_ == 'RAW':
  ...
  if cloud_db_ == 'DYNAMODB':
    node = raw.RawNode(data_dir=local_datadir_,address=('0.0.0.0',20303),link_no=link_no_,coin=curr_coin)
  ...
  if cloud_db_ == 'DYNAMODB':
    ...
    else: db_backup.ec2_reboot = ec2_reboot
  else:
    ...
    if local_datadir_ and s3db_bucket and s3db_path:
      db_backup = basenode.ZipBackup(os.path.join(local_datadir_,'zip'),s3db_bucket,s3db_path)
      # db_backup.readonly = True
      ...
      try:
        ...
        if secur_groups:
          ...
          if not exist30303 or not exist30302:
            permit = []
            if not exist30303:
              permit.append({'IpRanges':[{'CidrIp':'0.0.0.0/0'}],'FromPort':30303,'IpProtocol':'tcp','ToPort':30303})
            if not exist30302:
              permit.append({'IpRanges':[{'CidrIp':'0.0.0.0/0'}],'FromPort':30302,'IpProtocol':'udp','ToPort':30302})
  ...
  node = basenode.SyncNode(data_dir=local_datadir_,address=('0.0.0.0',30303),link_no=link_no_,coin=curr_coin)
