...
def get_script_address(pk_script, node, block=None):
  ...
  if tokens.match_template(TEMPLATE_PAY_TO_PUBKEY_HASH):
    ...
    hi,lo = divmod(block.link_no & 0xffff,256)
    ...
    while script:
      ...
      if opcode == opcodes.OP_0:
        ...
        if opcodes.OP_PUSHDATA1 <= opcode <= opcodes.OP_PUSHDATA4:
          ...
          fmt = ['<B', '<H', '<I'][iTmp]
          length = struct.unpack(fmt, script[:op_length])[0]
        ...
        if len(value) != length:
          raise Exception('not enought script for literal: ' + hexlify(sTmp).decode('latin-1') + ':' + hexlify(script).decode('latin-1'))
        opcode = Tokenizer.OP_LITERAL
